...
files2=`dotnet trglob '../more-examples/**/*.sql' | tr -d '\r' | grep -v '[.]errors$' | grep -v '[.]tree$' | grep -v '[.]trq$'`
files=()
for f in $files2
do
...
then
    gen=`find ../more-examples -type f -name '*.errors' -o -name '*.tree'`
    if [ "$gen" != "" ]
    then
        dos2unix -f $gen
    fi
